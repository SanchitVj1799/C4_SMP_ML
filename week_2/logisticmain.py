...
X = np.c_[np.ones((data.shape[0],1)), data[:,0:2]]  # adds column of ones (first argument) to X data (second argument) AND creates X from data at the same time...Concatenating data...
y = np.c_[data[:,2]]                                                          # creates Y from data.........concatenating data.........
...
print(h.shape)
plt.contour(xx1, xx2, h, [0.5], linewidths=2, colors='b')                                   #potting the function on the graph      
plt.show()
